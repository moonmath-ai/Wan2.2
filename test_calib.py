#!/usr/bin/env python3
"""Test LiteAttention with different calibrations, loading weights only once."""
...
def main():
    ...
    output_dir = Path(f"outputs/calib_test_{timestamp}")
    ...
    wan_i2v = wan.WanI2V(
        ...
        lite_attention_threshold=-10.0,  # initial, will change
    )
    ...
    for target in [0.01, 0.05, 0.1]:
        ...
        # Verify LiteAttention is available and targets is set correctly
        ...
        if sample_la is None:
            ...
            return
        print(f"Verified target_error in module: {sample_la.config.target_error}")
        ...
        for frame in [0, 10, 20]:
            ...
            frame_file = output_dir / f"target_{target}_frame0.png"
        ...
        results.append((target, gen_time, output_file, video_hash, skip_info))
        del video

    print(f"\n{'='*80}")
    print("SUMMARY")
    print('='*80)
    print(f"{'target':<12} {'Time (s)':<12} {'Tiles Skipped':<18} {'Hash':<22}")
    print('-'*80)
    for target, gen_time, output_file, video_hash, skip_info in results:
        print(f"{target:<12} {gen_time:<12.2f} {skip_info:<18} {video_hash:<22}")

    # Check if all hashes are the same
    hashes = [r[3] for r in results]
    if len(set(hashes)) == 1:
        print("\nWARNING: All video hashes are identical! targets may not be affecting output.")
    else:
        print("\nVideo hashes differ - targets are affecting output.")

    # Write summary to file
    summary_file = output_dir / "summary.txt"
    with summary_file.open("w") as f:
        f.write(f"LiteAttention target Comparison Test\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write(f"{'='*80}\n\n")
        f.write(f"{'target':<12} {'Time (s)':<12} {'Tiles Skipped':<18} {'Hash':<22}\n")
        f.write(f"{'-'*80}\n")
        for target, gen_time, output_file, video_hash, skip_info in results:
            f.write(f"{target:<12} {gen_time:<12.2f} {skip_info:<18} {video_hash:<22}\n")
        f.write(f"\n")
        if len(set(hashes)) == 1:
            f.write("WARNING: All video hashes are identical!\n")
        else:
            f.write("SUCCESS: Video hashes differ - targets are affecting output.\n")
